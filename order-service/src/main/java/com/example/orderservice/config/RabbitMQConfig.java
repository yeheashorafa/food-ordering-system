...
public class RabbitMQConfig {
    ...
    @Bean public Binding paymentBinding(Queue paymentQueue, TopicExchange orderEventsExchange) {
        return BindingBuilder.bind(paymentQueue).to(orderEventsExchange).with("order.placed");
    }
    @Bean public Binding deliveryBinding(Queue deliveryQueue, TopicExchange orderEventsExchange) {
        return BindingBuilder.bind(deliveryQueue).to(orderEventsExchange).with("order.ready.delivery");
    }
    @Bean public Binding paymentRefundBinding(Queue paymentRefundQueue, TopicExchange orderEventsExchange) {
        return BindingBuilder.bind(paymentRefundQueue).to(orderEventsExchange).with("order.cancelled");
    }
    @Bean public Binding deliveryCancelBinding(Queue deliveryCancelQueue, TopicExchange orderEventsExchange) {
        return BindingBuilder.bind(deliveryCancelQueue).to(orderEventsExchange).with("order.cancelled");
    }
    @Bean public Binding notificationCancelBinding(Queue notificationCancelQueue, TopicExchange orderEventsExchange) {
        return BindingBuilder.bind(notificationCancelQueue).to(orderEventsExchange).with("order.cancelled");
    }
    @Bean public Binding notificationBinding(Queue notificationQueue, TopicExchange deliveryEventsExchange) {
        return BindingBuilder.bind(notificationQueue).to(deliveryEventsExchange).with("delivery.assigned");
    }
    @Bean public Binding dlqBinding(Queue deadLetterQueue, TopicExchange deadLetterExchange) {
